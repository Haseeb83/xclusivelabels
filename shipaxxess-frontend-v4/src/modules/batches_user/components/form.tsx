...
const BatchNewForm = ({ addresses, packages, types }: BatchNewFormProps) => {
	...
	const onSubmit = async (data: Labels.BATCHZODSCHEMA) => {
		...
		const req = await api.url("/user/labels/singleLabel").useAuth().post(data);
	...
	const { data: couponData, refetch } = useCouponQuery(couponCode, !!couponCode) as any;

	const handleApplyCoupon = async () => {
		const { data } = (await refetch()) as any;
		if (!data?.value) return toast.error("Coupon does not exists");
	};
							...
							<Card className="p-6 space-y-2">
								<div className="flex items-center mb-5">
									<input
										...
										className="focus:outline-none w-full focus:ring-0 border rounded-lg p-2 rounded-r-none"
									/>
									<label
										onClick={handleApplyCoupon}
										className={`rounded-l-none  cursor-pointer bg-primary text-white px-3 py-2 rounded-r-lg ${
											couponApplied && "opacity-80"
										}`}>
										{!couponApplied ? "Apply" : "Applied"}
									</label>
								</div>
